...
public :
    ...
    unsigned int hash_function(string name)
    {
        ...
        for (i = 0; i < len; i++)
        {
            hash = ((name[i]) + (hash << 6) + (hash << 16) - hash)%totNum_buckets;
        }
    ...
    symbolInfo* look_up(string name, ofstream& outfile, int check = 1)
    {
        ...
        while(temp != NULL)
        {
            if(temp->getName() == name)
            {
                if(check == 1)
                {
                    cout<<"\t"<<"'"<<name<<"' found in ScopeTable# "<<uniq_id<<" at position " <<bucket_number+1<<", "<<position<<endl;
                    outfile<<"\t"<<"'"<<name<<"' found in ScopeTable# "<<uniq_id<<" at position " <<bucket_number+1<<", "<<position<<endl;
                }
                return temp;
            }
            position++;
    ...
    bool Insert(symbolInfo& symbol, ofstream& outfile)
    {
        ...
        if(temp == NULL)
        {
            ...
            cout<<"\t"<<"Inserted in ScopeTable# "<<uniq_id<<" at position " <<bucket_number+1<<", "<<position<<endl;
            outfile<<"\t"<<"Inserted in ScopeTable# "<<uniq_id<<" at position " <<bucket_number+1<<", "<<position<<endl;
            return true;
        }
    ...
    bool Delete(string name, ofstream& outfile)
    {
        ...
        if(temp->getName() == name)
        {
            ...
            cout<<"\t"<<"Deleted '"<<name<<"' from ScopeTable# "<<uniq_id<<" at position " <<bucket_number+1<<", "<<position<<endl;
            outfile<<"\t"<<"Deleted '"<<name<<"' from ScopeTable# "<<uniq_id<<" at position " <<bucket_number+1<<", "<<position<<endl;
            return true;
        }
...
public:
    ...
    void enter_scope(int uniq_id,int totNum_buckets,ofstream& outfile)
    {
        ...
        cout<<"\t"<<"ScopeTable# "<<uniq_id<<" created"<<endl;
        outfile<<"\t"<<"ScopeTable# "<<uniq_id<<" created"<<endl;
