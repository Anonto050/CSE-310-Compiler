...
public :
    ...
    void setOffset(int stack_offset){
...
public :
    ...
    bool Insert(symbolInfo& symbol, ofstream& outfile, int idInScope_total = -1)
    {
        int stack_offset = -((idInScope_total+1)*2);
...
public:
    ...
    void reset_currentScopeIdCount(){
        currentScopeIdCount = 0;
    }

    int get_currentScopeIdCount(){
        return currentScopeIdCount;
    }
    ...
    bool Insert(symbolInfo& symbol, ofstream& outfile)
    {
        ...
        bool temp = curr_scope->Insert(symbol, outfile, currentScopeIdCount);

        if(temp == true && symbol.getIsParameter() == false && symbol.getType() != "FUNCTION")
        {
            int array_size = symbol.getArraySize();

            currentScopeIdCount += array_size ? array_size : 1;
        }
